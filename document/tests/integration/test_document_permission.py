...
def test_document(test_users):
    doc = baker.make(
        Document, owner=test_users["owner"], public_premission_access=False
    )
    ...
    baker.make(
        ...
        document=doc,
        access_level=AccessLevel.PERMISSION_MAP["Admin"],
    )
    baker.make(
        AccessLevel,
        user=test_users["writer"],
        document=doc,
        access_level=AccessLevel.PERMISSION_MAP["Writer"],
    )
    baker.make(
        AccessLevel,
        user=test_users["reader"],
        document=doc,
        access_level=AccessLevel.PERMISSION_MAP["ReadOnly"],
    )
    return doc
...
class TestDocumentPermissionEndpoints:
    def test_set_permission_by_owner(
        self, api_client, base_permission_url, test_users, test_document
    ):
        ...
        response = api_client.post(
            f"{base_permission_url}set_permission/", data=data, format="json"
        )

        assert response.status_code == status.HTTP_200_OK
        assert (
            AccessLevel.objects.get(
                user=test_users["other"], document=test_document
            ).access_level
            == AccessLevel.PERMISSION_MAP["Writer"]
        )
        assert (
            AccessLevel.objects.get(
                user=test_users["reader"], document=test_document
            ).access_level
            == AccessLevel.PERMISSION_MAP["Admin"]
        )

    def test_set_permission_by_admin(
        self, api_client, base_permission_url, test_users, test_document
    ):
        api_client.force_authenticate(user=test_users["admin"])
        data = {
            "document": test_document.id,
            "permissions": [{"user": test_users["other"].id, "permission": "ReadOnly"}],
            "send_email": False,
        }
        ...
        assert (
            ...
            == AccessLevel.PERMISSION_MAP["ReadOnly"]
        )

    def test_set_permission_denied_by_writer(
        self, api_client, base_permission_url, test_users, test_document
    ):
    ...
    def test_get_permission_list_by_owner(
        self, api_client, base_permission_url, test_users, test_document
    ):
        api_client.force_authenticate(user=test_users["owner"])

        response = api_client.get(
            f"{base_permission_url}get_permission_list/{test_document.id}/"
        )
    ...
    def test_get_user_permission_for_reader(
        self, api_client, base_permission_url, test_users, test_document
    ):
        api_client.force_authenticate(user=test_users["reader"])

        response = api_client.get(
            f"{base_permission_url}get_user_permission/{test_document.id}/"
        )
    ...
    def test_get_user_permission_default(
        self, api_client, base_permission_url, test_users, test_document
    ):
